"""
TARGETED SWEEP — base only
Run in its own terminal: python -m modal run sweep_base.py --detach
...
def main():
    ...
    print("Running base...")
    results = run.remote()
    fn = f"sweep_base_{ts}.json"
    with open(fn,"w") as f:
        json.dump({"model":"base","timestamp":ts,
