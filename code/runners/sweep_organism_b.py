"""
TARGETED SWEEP — organism_b only
Run in its own terminal: python -m modal run sweep_organism_b.py --detach
...
app    = modal.App("sl-sweep-organism_b")
...
def main():
    ...
    print("Running organism_b...")
    results = run.remote()
    fn = f"sweep_organism_b_{ts}.json"
    with open(fn,"w") as f:
        json.dump({"model":"organism_b","timestamp":ts,
